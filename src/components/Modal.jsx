...
const Modal = ({
  ...
  return (
    <section
      className={` z-30 fixed top-0 left-0 bottom-0 right-0 bg-black/40 flex justify-center items-center transition-opacity ${
        isShowForm ? "opacity-100 visible" : " opacity-0 invisible"
      }`}
    >
      <form
        onSubmit={handleSubmit(submit)}
        className="form-modal bg-white p-4 grid gap-4 w-[300px]   relative"
      >
        <div className="">
          ...
          <div className="grid gap-1">
            ...
            <input
              required
              maxLength={20}
              className="border-[1px] border-gray-400 rounded-sm bg-gray-100 p-1 "
              id="first_name"
              type="text"
              {...register("first_name")}
            />
          ...
          ></i>
          <button className="bg-purple-p text-white p-2 w-full hover:bg-purple-p/90 transition-colors">
